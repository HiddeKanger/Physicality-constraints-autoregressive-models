...
def _make_slices_begin_size(input_, slices):
    ...
    for i, s in enumerate(slices):
        if s is Ellipsis:
            if not before_ellipsis:
                raise ValueError(
                    'Cannot use more than one ellipsis in slice spec.')
        ...
        start = tf.cond(tf.convert_to_tensor(start >= 0),
                        lambda: start, lambda: start + dim_size)
        stop = tf.cond(tf.convert_to_tensor(stop >= 0),
                       lambda: stop, lambda: stop + dim_size)
    ...
    size_mask = tf.scatter_nd(dim_idx, tf.ones_like(
        sizes, dtype=tf.bool), [inp_rank])
