...
def ncon(AA, v, order=None, forder=None, check_indices=True):
    ...
    if hasattr(AA, "shape"):
        ...
        AA = list(AA)
    v = list(v)
    if not isinstance(v[0], collections_abc.Iterable):
...
def connect_graph(AA, v, order):
    ...
    while unvisited:
        ...
        while to_visit:
            ...
            i_inds = set(v[i])
            neighs = (j for j, j_inds in enumerate(v)
                      if i_inds.intersection(j_inds))
            for neigh in neighs:
...
####################################################################
def ensure_complex(tensor):
    """Ensure the tensor is of complex type."""
    if tensor.dtype in [tf.float32, tf.float64]:
        # Convert float tensors to complex tensors with zero imaginary part
        return tf.complex(tensor, tf.zeros_like(tensor))
    elif tensor.dtype in [tf.complex64, tf.complex128]:
        return tensor
    else:
        raise TypeError("Unsupported tensor type: {}".format(tensor.dtype))


def complex_tensordot(A, B, inds):
    """Perform tensordot between two tensors, ensuring they are complex."""
    A_complex = tf.cast(ensure_complex(A), tf.complex64)
    B_complex = tf.cast(ensure_complex(B), tf.complex64)
    return tf.tensordot(A_complex, B_complex, inds)


def con(A, B, inds):
    if type(A) == type(B) == np.ndarray:
        return np.tensordot(A, B, inds)
    elif tf.is_symbolic_tensor(A) or tf.is_symbolic_tensor(B):
        return complex_tensordot(A, B, inds)
    else:
        return A.dot(B, inds)
